...
export function Header() {
  ...
  const activePillStyle: React.CSSProperties = {
    background: "#e22222", color: "#fff", borderRadius: "20px",
  ...
  const inactiveLinkStyle: React.CSSProperties = {
    color: "rgba(255,255,255,0.75)", borderRadius: "20px",
    padding: "7px 18px", fontSize: 13, fontWeight: 600,
    background: "transparent", transition: "color 0.18s",
    textDecoration: "none", display: "inline-block",
  };

  const renderLogo = () => {
    if (loadingLogo) return <div style={{ width: 62, height: 62, background: "rgba(255,255,255,0.1)", borderRadius: 8, animation: "pulse 1.5s ease-in-out infinite" }} />;
    if (logo?.type === "image" && logo.imageUrl) return <img src={logo.imageUrl} alt={logo.alt || "360 ArtDesign Logo"} style={{ height: 105, width: "auto", maxWidth: 180, objectFit: "contain", display: "block" }} />;
    return <div style={{ width: 55, height: 55, background: "radial-gradient(circle, #cc1111 20%, #7a0000 100%)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, boxShadow: "0 0 16px rgba(200,20,20,0.45)" }}>{logo?.icon || "🐺"}</div>;
  };

  // Baaki return same rahega — sirf handleMouseEnter/Leave change hua hai
  return (
    <>
      <style>{`
